...
hongheitool.MIN_HU_XI = 15

hongheitool.HUPAI_TYPE = {
	liuju = -1, --流局
	normal = 1, --平胡
	tianHu = 2, --天胡
	diHu = 3, --地胡
...
function hongheitool.findCard(tbl,value)
...
function hongheitool.delCard(cards,pcard)
...
function checkXiaoQiDui(user,cards)
	...
	if duiCount == 7 then
		return hongheitool.HUPAI_TYPE.xiaoQiDuiHu
	end
	for _,info in pairs(user.menzi) do
		if info.type == hongheitool.cardType.she then
			sheCount = sheCount + 1
		end
	end
	if sheCount == 2 then
		return hongheitool.HUPAI_TYPE.shuangLongHu
	end
end

--获取听牌信息
function hongheitool.getTingPaiInfo(user)
	return huZiGameHu.getTingPaiInfo(user,paoHuTingpaiLogic,hongheitool.MIN_HU_XI)
end
...
function hongheitool.checkPaoHuZiHu(user,cards,pcard,isDraw)
	...
	-- for _,info in pairs(user.menzi) do
	-- 	if info.type == hongheitool.cardType.long or info.type == hongheitool.cardType.pao then
	-- 		daiDingHuType = true
	-- 		break
	-- 	end
	-- end
	...
	-- 			if #itemInfo == 1 and isDraw and hongheitool.getValueCount(cards,pcard) == 2 then --单调的话 并且自己抓的话 并且自己有两张这种牌的话 直接不让胡 扫烂了
	-- 				isAppend = false
	-- 			end
	...
	-- 			end
	-- 		end
	-- 		if hongheitool.getValueCount(cards,pcard) == 2 and isDraw then
	-- 			if #twoIemtList == 2 then 
	-- 				if twoIemtList[1][1] == twoIemtList[1][2] and twoIemtList[2][1] == twoIemtList[2][2] then--先判断是不是两对
	-- 					if hongheitool.getValueCount(cards,pcard) == 3 then
	-- 						isAppend = false
	-- 					end
	...
	-- 		if isAppend then
	-- 			for _,itemInfo in pairs(item[2]) do
	-- 				if #itemInfo == 3 and itemInfo[1] == itemInfo[2] and itemInfo[2] == itemInfo[3] then
	-- 					if itemInfo[1] == pcard then
	-- 						if not daiDingHuType then
	-- 							daiDingHuType = true
	-- 						end
	-- 					end
	-- 				end
	-- 				if #itemInfo == 2 and itemInfo[1] == itemInfo[2] and daiDingHuType then
	-- 					huPaiDuoYu = 3
	-- 					isNomal = true
	-- 				end
	-- 				if #itemInfo == 2 and itemInfo[1] == itemInfo[2] and itemInfo[1] == pcard then
	-- 					if math.floor(pcard/100) == 1 then
	-- 						huPaiDuoYu = 2
	-- 					else
	-- 						huPaiDuoYu = 3
	-- 					end
	...
	-- 				if not isDraw then
	-- 				    if isNomal then
	-- 						handHuXi = handHuXi - huPaiDuoYu
	-- 					end
	-- 				end
	-- 				print("-----handHuXi+user.roundHuXi-----",handHuXi+user.roundHuXi)
	-- 				if handHuXi+user.roundHuXi >= hongheitool.MIN_HU_XI then
	...
	-- 					--return isNomal,handHuXi+user.roundHuXi,handHuCards
	-- 				end
	-- 			end
	-- 		end
	-- 	end
	...
	-- local type = nil
	-- for _,info in pairs(user.menzi) do
	-- 	if info.type == hongheitool.cardType.peng and info.card == pcard then
	-- 		table.insert(cards,info.card)
	-- 		table.insert(cards,info.card)
	-- 		table.insert(cards,info.card)
	-- 		duoyu = hongheitool.getCardsTypeHuXi(info)
	-- 		removeCards = pcard
	-- 		type = info.type
	-- 		break
	-- 	end

	-- 	if info.type == hongheitool.cardType.kan and info.card == pcard then
	-- 		table.insert(cards,info.card)
	-- 		table.insert(cards,info.card)
	-- 		table.insert(cards,info.card)
	-- 		duoyu = hongheitool.getCardsTypeHuXi(info)
	...
	-- 			if #twoIemtList == 2 then 
	-- 				if twoIemtList[1][1] == twoIemtList[1][2] and twoIemtList[2][1] == twoIemtList[2][2] then--先判断是不是两对
	-- 					if hongheitool.getValueCount(cards,pcard) == 3 and type == hongheitool.cardType.kan then
	-- 						isAppend = false
	-- 					end
	-- 				else --如果只有一对 如 202,202  207,210 那么就只能扫
	-- 					if twoIemtList[1][1] == twoIemtList[1][2] and twoIemtList[1][1] == pcard then
	...
	-- 					end
	-- 				end
	-- 				if #itemInfo == 2 and itemInfo[1] == itemInfo[2] and daiDingHuType then --检测跑胡
	-- 					if type == hongheitool.cardType.peng then --如果是碰 不管是不是自摸都减3分
	-- 						huPaiDuoYu = 3
	-- 					else
	-- 						if not isDraw then --如果是坎的 并且不是自摸 减3分
	-- 							huPaiDuoYu = 3
	-- 						end
	-- 					end
	-- 					local sb = math.floor(pcard/100)

	-- 					if type == hongheitool.cardType.kan then
	-- 						if sb == 1 then --小坎 再减3分
	-- 							huPaiDuoYu = huPaiDuoYu + 3
	-- 						else
	-- 							huPaiDuoYu = huPaiDuoYu + 6
	-- 						end
	-- 					end
	-- 					if type == hongheitool.cardType.peng then
	-- 						if sb == 1 then --小碰 再减1分
	-- 							huPaiDuoYu = huPaiDuoYu + 1
	-- 						else
	-- 							huPaiDuoYu = huPaiDuoYu + 3
	-- 						end
	-- 					end
	...
	-- 					isNomal = true
	-- 				end
	-- 				if #itemInfo == 2 and itemInfo[1] == itemInfo[2] and itemInfo[1] == pcard then --碰胡
	-- 					if not isDraw then --不是自摸先减除3分 当做跑
	-- 						local sb = math.floor(pcard/100)
	-- 						if sb == 1 then --
	-- 							huPaiDuoYu = 2
	-- 						else
...
--获取对应牌型的胡息
function hongheitool.getCardsTypeHuXi(info)
	--判断吃 是不是1510
	local infoTmp = table.copy(info)
	local sb = 0
	if infoTmp.type == hongheitool.cardType.chi then
		sb = math.floor(infoTmp.data[1] / 100)
	  	return checkChiHuxi(infoTmp.data,sb)
	else
		sb = math.floor(infoTmp.card / 100)
		return hongheitool.HUPAI_HU_XI[sb][infoTmp.type]
	end
end

--检测坎跑
function hongheitool.checkKanPao(user,pcard)
	for _,info in pairs(user.menzi) do
		if info.type == hongheitool.cardType.kan and pcard == info.card then
			return true,hongheitool.cardType.pao
		end
	end
	if hongheitool.getValueCount(user.handInCards,pcard) == 3 then
		return true,hongheitool.cardType.handpao
	end
end

--检测踢龙
function hongheitool.checkTiLong(user,pcard)
	for _,info in pairs(user.menzi) do
		if info.type == hongheitool.cardType.kan and pcard == info.card then
			return true,hongheitool.cardType.long
		end
	end
	if hongheitool.getValueCount(user.handInCards,pcard) == 3 then
		return true,hongheitool.cardType.handlong
	end
end

--检测p碰跑
function hongheitool.checkPengPao(user,pcard)
	for _,info in pairs(user.menzi) do
		if info.type == hongheitool.cardType.peng and pcard == info.card then
			return true
		end
	end
end

function hongheitool.getValueCount(cards,card)
	local tables = {}
	for i = 1,#cards do
		if not tables[cards[i]] then
			tables[cards[i]] = 1
...
--检测有没有蛇
function hongheitool.checkHandShe(cards)
	local tables = {}
	for i = 1,#cards do
		if not tables[cards[i]] then
			tables[cards[i]] = 1
...
--检测碰
function hongheitool.checkPeng(user,pcard)
	if hongheitool.findCard(user.notPengPai,pcard) then
		return nil
	end
	local tables = {}
	...
	for card, count in pairs(tables) do
		if card == pcard and count == 2 then
			return hongheitool.cardType.peng
		end
	end
end

--检测sao
function hongheitool.checkSao(user,pcard)
	local tables = {}
	for i = 1,#user.handInCards do
		if not tables[user.handInCards[i]] then
			tables[user.handInCards[i]] = 1
	...
	for card, count in pairs(tables) do
		if card == pcard and count == 2 then
			return hongheitool.cardType.kan
		end
	end
end
...
local function getLuo(chiCardTypeList,cards,pcard,luoCount)
	...
	for _, info in pairs(chiCardTypeList) do
		if hongheitool.findCard(cards, info[1]) and hongheitool.findCard(cards, info[2]) then
			local data
			if info[1] == info[2] then
				if hongheitool.getValueCount(cards,info[1]) == 2 then
					local data = {}
					data[1] = info[1]
					data[2] = info[2]
					data[3] = pcard
			...
			elseif  pcard == info[2] then
				if  hongheitool.getValueCount(cards,info[2]) == 2 then
					local data = {}
					data[1] = info[1]
					data[2] = info[2]
					data[3] = pcard
				...
				data[3] = pcard
				table.insert(luoData,data)
				if hongheitool.getValueCount(cards,info[1]) == 2 and hongheitool.getValueCount(cards,info[2]) == 2 and hongheitool.getValueCount(cards,pcard) == 2 then
					local data = {}
					data[1] = info[1]
					data[2] = info[2]
					data[3] = pcard
...
--检测吃
function hongheitool.checkChi(user,pcard,suser)
	...
	for _, card in pairs(cards) do
		local count = hongheitool.getValueCount(cards,card)
		if count > 2 then
	...
	for card,count in pairs(kanOrSheCardList) do
		for i = 1, count do
			hongheitool.delCard(cards,card)
		end
	end

	if hongheitool.findCard(user.notChiPai,pcard) then
		return nil
	end
	...
	if hongheitool.findCard(suser.qipai,pcard) then
		return nil
	end
	...
	for _, info in pairs(chiCardTypeList) do
		if hongheitool.findCard(cards, info[1]) and hongheitool.findCard(cards, info[2]) then
			--先查找是否存在吃的牌
			local count = hongheitool.getValueCount(cards,pcard)
			if count == 1 then
				if info[1] == pcard or info[2] == pcard then
					local handInCards = table.copy(cards)
					hongheitool.delCard(handInCards,info[1])
					hongheitool.delCard(handInCards,info[2])
					local chis = {}
					...
					if info[1] == info[2] then
						if hongheitool.getValueCount(cards,info[1]) == 2 then --说明有一对相反的牌
							local tmpCards = table.copy(cards)
							hongheitool.delCard(tmpCards,info[1])
							hongheitool.delCard(tmpCards,info[2])
			...
			elseif count == 2 then
				if info[1] == info[2] then
					...
					if info[1] == pcard then
						local tmpCards = table.copy(cards)
						hongheitool.delCard(tmpCards,pcard)
						hongheitool.delCard(tmpCards,info[1])
						hongheitool.delCard(tmpCards,info[2])
						local luoData  = getLuo(chiCardTypeList,tmpCards,pcard,1)
						if luoData then
					...
					elseif info[2] == pcard then
						...
						if luoData and #luoData > 0 then
							...
							chis.handInCards = handInCards
							chis.chiData = info
							if hongheitool.findCard(cards, tmpCard) then
								if #luoData == 1 then
									chis.luoCount = 1
									chis.isChoose = 0
								else
...
function hongheitool.checkScore(cards,huxi,difen,beishu,hcard)
...
function hongheitool.checkDistance(lat,lng,users)
...
function hongheitool.checkIp(ip,users)
	for _, user in pairs(users) do
		if user.ip ==  ip then
			return nil
		end
...
function hongheitool.setHuXi(user)
	local roundHuXiTmp = 0
	local ohterRoundHuXiTmp = 0
	for _, info in pairs(user.menzi) do
		local huXi = hongheitool.getCardsTypeHuXi(info) or 0
		roundHuXiTmp = roundHuXiTmp + huXi
		if info.type ~= hongheitool.cardType.kan and info.type ~= hongheitool.cardType.she and info.type ~= hongheitool.cardType.long then
...
function hongheitool.jisuanXY(users)
	local locatingList = {}
