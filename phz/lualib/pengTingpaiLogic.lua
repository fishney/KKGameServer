local pengHuTingpaiLogic = {}

pengHuTingpaiLogic.AllPaiValue = 
{
	101,102,103,104,105,106,107,108,109,110,
	201,202,203,204,205,206,207,208,209,210
}

pengHuTingpaiLogic.resIndex =
{
    huxi = 1,
    combi = 2
}
...
function pengHuTingpaiLogic.getTingPaiRes(_handPokers,tihuFunc,tihuFunc,tihuFuncParame3)
    local res_ting_hu = {}
    local res = pengHuTingpaiLogic.getTingPaiList(_handPokers,res_ting_hu,0,{})
...
function pengHuTingpaiLogic.newTingPaiResStruct()
    local ret = {}
    ret.isCanTing = false
    ret.isCantihu = false
    ret.maxTempHuxi = -1
...
function pengHuTingpaiLogic.updateTingPaiResStruct(oldres,newres)
...
function pengHuTingpaiLogic.getTingPaiList(_handPokers,res_ting_hu,huxi,headcombi) --headcombi {{101,101,101},{202,202,202}}

    local handPokers    = table.clone(_handPokers)
    local kindNum       = pengHuTingpaiLogic.getCountAndDelByHand(handPokers,g_phzCards.kind_CardValue)
    local xiaoQiang     = pengHuTingpaiLogic.resolvexiaoQingPai(handPokers)
    local xiaoQiangHuxi = pengHuTingpaiLogic.getxiaoQingHuxi(xiaoQiang) + huxi
    local res = pengHuTingpaiLogic.newTingPaiResStruct()
    local newHeadCombi = {}
    newHeadCombi = pengHuTingpaiLogic.GetNewconnectCombis(xiaoQiang,headcombi)  --python_zhushi
    local combis = {}
    if (#handPokers < 3) then
        combis[#combis + 1] = {}
        combis[1][#combis[1] + 1] = handPokers
    else
        combis = pengHuTingpaiLogic.getAllCardCombi(handPokers,kindNum)
    end

    local markTempList = {}
    if kindNum > 0 then
        local tempRet = pengHuTingpaiLogic.kindPaiBuTiPai(_handPokers,res_ting_hu,xiaoQiang,kindNum,huxi,headcombi)
        pengHuTingpaiLogic.updateTingPaiResStruct(res,tempRet)
        for  i = 1, #combis do
            local lastIndex = #combis[i]
            local tempNewHeadCombi = {}
            tempNewHeadCombi = pengHuTingpaiLogic.GetNewconnectCombis(newHeadCombi,combis[i])
            table.remove(tempNewHeadCombi,#tempNewHeadCombi)               --最后一组不是有效的
            if #combis[i][lastIndex] < kindNum * 2 then                                    
				local tempHufen = pengHuTingpaiLogic.GetvalidCombiHufen(combis[i]) + xiaoQiangHuxi
                table.sort(combis[i][lastIndex])
                local tempRet = pengHuTingpaiLogic.getKindPai_ting_hu(combis[i][lastIndex],res_ting_hu, kindNum, tempHufen,tempNewHeadCombi)
                pengHuTingpaiLogic.updateTingPaiResStruct(res,tempRet)
            elseif #combis[i][lastIndex] <= kindNum * 2 + 4 then
               local tempRet = pengHuTingpaiLogic.replaceKindPaiDnfTingpai(combis[i][lastIndex],res_ting_hu,kindNum,xiaoQiangHuxi,tempNewHeadCombi)
                pengHuTingpaiLogic.updateTingPaiResStruct(res,tempRet)
            end
		end
    else
        ...
        for i,combi in ipairs(combis) do
            local tempNewHeadCombi = pengHuTingpaiLogic.GetNewconnectCombis(newHeadCombi,combis[i])
            table.remove(tempNewHeadCombi,#tempNewHeadCombi)               --最后一组不是有效的
            tempHufen = pengHuTingpaiLogic.GetvalidCombiHufen(combi) + xiaoQiangHuxi
            if #combi[#combi] == 2 then
                local tempisCanTing = pengHuTingpaiLogic.getchuTingPairByTwo(combi[#combi], res_ting_hu, tempHufen,tempNewHeadCombi)
            ...
            elseif #combi[#combi] == 4 then
                local tempisCanTing = pengHuTingpaiLogic.getchuTingPairByFour(combi[#combi], res_ting_hu, tempHufen,tempNewHeadCombi)
                res.isCanTing = res.isCanTing or tempisCanTing
            elseif #combi[#combi] == 1 then
                local tempisCanTing = pengHuTingpaiLogic.getchuTingPairByOne(combi[#combi], res_ting_hu, tempHufen,tempNewHeadCombi)
                res.isCanTing = res.isCanTing or tempisCanTing
            else
    ...
    if res.isCantihu then
        for _indexXiao,_valueXiao in ipairs(xiaoQiang) do
            if #_valueXiao == 3 then
                if pengHuTingpaiLogic.getPaiType(_valueXiao[1]) == 2 then
                    pengHuTingpaiLogic.setTing_huValue(res_ting_hu,_valueXiao[1],res.maxTempHuxi + (g_phzHuxi.b_qing - g_phzHuxi.b_xiao),res.tiHuCombi)
                    res.isCanTing = true
                else
                    pengHuTingpaiLogic.setTing_huValue(res_ting_hu,_valueXiao[1],res.maxTempHuxi + (g_phzHuxi.s_qing - g_phzHuxi.s_xiao),res.tiHuCombi)
                    res.isCanTing = true
                end
            end
        end
...
function pengHuTingpaiLogic.GetNewconnectCombis(combione,combitwo)
...
function pengHuTingpaiLogic.kindPaiBuTiPai(handPokers,res_ting_hu,xiaoQiang,kindNum,huxi,headcombi)
    local res = pengHuTingpaiLogic.newTingPaiResStruct()
    if #xiaoQiang == 0 then
        return res
    end
    ...
    for _index,paiCombi in ipairs(xiaoQiang) do
        if #paiCombi == 3 then
            if pengHuTingpaiLogic.getPaiType(paiCombi[1]) == 1 then
    ...
    for _index, paivalue in ipairs(xiaoPaiValue) do
        if kindNum > 0 then
            ...
            local tempRet = pengHuTingpaiLogic.getTingPaiList(temphandpokers,res_ting_hu,huxi,headcombi)
            pengHuTingpaiLogic.updateTingPaiResStruct(res,tempRet)
...
function pengHuTingpaiLogic.getKindPai_ting_hu(handpokers,res_ting_hu, kindNum, tempHuxi,headcombi)
    local handSize = #handpokers + kindNum;
	if handSize % 3 == 1 then    --单钓
        if kindNum == 1 then
            return pengHuTingpaiLogic.DOneKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
        elseif kindNum == 2 then
            return pengHuTingpaiLogic.DTwoKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
        elseif kindNum == 3 then
            return pengHuTingpaiLogic.DThreeKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
        elseif kindNum == 4 then
            return pengHuTingpaiLogic.DFourKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
        end

    else
        if kindNum == 1 then
            return pengHuTingpaiLogic.SOneKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
        elseif kindNum == 2 then
            return pengHuTingpaiLogic.STwoKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
        elseif kindNum == 3 then
            return pengHuTingpaiLogic.SThreeKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
        elseif kindNum == 4 then
            return pengHuTingpaiLogic.SFourKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
        end
    end
end

function pengHuTingpaiLogic.DOneKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
    headcombi[#headcombi + 1] = {}
    for i,paiValue in ipairs(pengHuTingpaiLogic.AllPaiValue) do
        headcombi[#headcombi] = {paiValue}
        pengHuTingpaiLogic.setTing_huValue(res_ting_hu, paiValue, tempHuxi,headcombi)
    end
    local res = pengHuTingpaiLogic.newTingPaiResStruct()
    res.isCanTing = true
    return res
...
function pengHuTingpaiLogic.DTwoKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
	...
	for i = 1, #handpokers do
		type[i] = pengHuTingpaiLogic.getPaiType(handpokers[i])
		value[i] = pengHuTingpaiLogic.getPaiValue(handpokers[i])
    end
    pengHuTingpaiLogic.setChuTingCard(handpokers, tempChuTing, value, type, 1, 2, 0,{})
    local size = 0
    local maxTempHuxi = -1
    local maxKindTipai = 0
    for i,v in pairs(tempChuTing) do
        size = 1
        if v[pengHuTingpaiLogic.resIndex.huxi] > maxTempHuxi then
            maxTempHuxi = v[pengHuTingpaiLogic.resIndex.huxi]
            maxKindTipai = i
        end
    end
	...
	if size > 0 then
        ...
        for i,paiValue in ipairs(pengHuTingpaiLogic.AllPaiValue) do
            ...
            pengHuTingpaiLogic.setTing_huValue(res_ting_hu, paiValue, tempHuxi + maxTempHuxi,headcombi)
        end
        res.isCanTing = true
	else
        headcombi[#headcombi + 1] = {}
        ...
        pengHuTingpaiLogic.SOneKindPaiTingRes(temComs1,res_ting_hu,tempHuxi,headcombi)
        headcombi[#headcombi] = {handpokers[2],handpokers[2]}
        pengHuTingpaiLogic.SOneKindPaiTingRes(temComs2,res_ting_hu,tempHuxi,headcombi)
        res.isCanTing = true
    end
    return res
end

function pengHuTingpaiLogic.DThreeKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
    ...
    if handSize == 1 then
        ...
        for i,paiValue in ipairs(pengHuTingpaiLogic.AllPaiValue) do
            headcombi[#headcombi][1] = paiValue
            pengHuTingpaiLogic.setTing_huValue(res_ting_hu, paiValue, tempHuxi + tempTypeHuxi[type],headcombi)
        end
        res.isCanTing = true
    elseif handSize == 4 then
		...
		for i,comb in ipairs(indexValue) do
			local tempChuTing1 = {} 
			local tempChuTing2 = {}
			pengHuTingpaiLogic.setChuTingCard(handpokers, tempChuTing1, value, type, comb[1], comb[2], 0,{});
            pengHuTingpaiLogic.setChuTingCard(handpokers, tempChuTing2, value, type, comb[3], comb[4], 0,{});
            local size1 = 0
            ...
            for ting1,hu1 in pairs(tempChuTing1) do
                size1 = 1
                if hu1[pengHuTingpaiLogic.resIndex.huxi] > maxTempHuxi1 then
                    maxTempHuxi1 = hu1[pengHuTingpaiLogic.resIndex.huxi]
                    maxKindtiPai1 = ting1
                end
            end
            for ting2,hu2 in pairs(tempChuTing2) do
                size2 = 1
                if hu2[pengHuTingpaiLogic.resIndex.huxi] > maxTempHuxi2 then
                    maxTempHuxi2 = hu2[pengHuTingpaiLogic.resIndex.huxi]
                    maxKindtiPai2 = ting2
                end
            end
            ...
            if size1 > 0 and size2 > 0 then     --能解决2对 剩一张赖子听所有牌
                ...
                for i,paiValue in ipairs(pengHuTingpaiLogic.AllPaiValue) do
                    headcombi[#headcombi][1] = paiValue
                    pengHuTingpaiLogic.setTing_huValue(res_ting_hu, paiValue, tempHuxi + maxTempHuxi1 + maxTempHuxi2,headcombi)
                end
                res.isCanTing = true
...
function pengHuTingpaiLogic.DFourKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
	...
	local type = 0
    if handSize == 0 then
        pengHuTingpaiLogic.DOneKindPaiTingRes(handpokers, res_ting_hu, tempHuxi + tempTypeHuxi[2] )
        res.isCanTing = true
    elseif handSize == 3 then
		...
		for i,comb in ipairs(indexValue) do
			local tempChuTing = {}
            pengHuTingpaiLogic.setChuTingCard(handpokers, tempChuTing, value, type, comb[1], comb[2], 0)
            local size = 0
            local maxTempHuxi = -1
            local maxKindtiPai = 0
            for ting,hu in pairs(tempChuTing) do
                size = 1
                if hu[pengHuTingpaiLogic.resIndex.huxi] > maxTempHuxi then
                    maxTempHuxi = hu[pengHuTingpaiLogic.resIndex.huxi]
                    maxKindtiPai = ting
                end
            end
			if size > 0 then
                ...
                for i,paiValue in ipairs(pengHuTingpaiLogic.AllPaiValue) do
                    headcombi[#headcombi] = {paiValue}
                    pengHuTingpaiLogic.setTing_huValue(res_ting_hu, paiValue, tempHuxi + maxTempHuxi + tempTypeHuxi[type[comb[3]]],headcombi)
                end
			end
			...
			pengHuTingpaiLogic.setTing_huValue(res_ting_hu, handpokers[comb[3]], tempHuxi + tempTypeHuxi[type[comb[1]]] + tempTypeHuxi[type[comb[2]]],headcombi)
		end
        res.isCanTing = true
    elseif handSize == 6 then
        local kindNum = 3
		local tempHandPoker = table.clone(handpokers)
		local markTempl = pengHuTingpaiLogic.getmarkTempl(tempHandPoker)
        if #markTempl >= kindNum then
            tempHandPoker[#tempHandPoker + 1] = g_phzCards.kind_CardValue
			return pengHuTingpaiLogic.markDnfTingCard(markTempl, tempHandPoker, res_ting_hu, kindNum, tempHuxi,headcombi)
        end
    end
    return res
end

function pengHuTingpaiLogic.SOneKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,_headcombi)
    local tempTypeHuxi = { g_phzHuxi.s_xiao,g_phzHuxi.b_xiao}
	--到这肯定是 1张王 和 剩一张牌
	local type = pengHuTingpaiLogic.getPaiType(handpokers[1])
	local value = pengHuTingpaiLogic.getPaiValue(handpokers[1])
	...
	if (value > 1) then
		...
		pengHuTingpaiLogic.setTing_huValue(res_ting_hu, handpokers[1] - 1, tempHuxi,headcombi)
    end

	if (value > 2) then
        headcombi[#headcombi][2] = handpokers[1] - 1      --value = 3 听 1 补 2
		pengHuTingpaiLogic.setTing_huValue(res_ting_hu, handpokers[1] - 2, tempHuxi,headcombi)
    end
	...
	if (value == 2) then
		...
		pengHuTingpaiLogic.setTing_huValue(res_ting_hu, type * 100 + 7, tempHuxi + tempTypeHuxi[type],headcombi)
        headcombi[#headcombi][2] = type * 100 + 7              --value = 2 听 10 补 7
		pengHuTingpaiLogic.setTing_huValue(res_ting_hu, type * 100 + 10, tempHuxi + tempTypeHuxi[type],headcombi)
	elseif (value == 7) then
        headcombi[#headcombi][2] = type * 100 + 10              --value = 7 听 2 补 10
		pengHuTingpaiLogic.setTing_huValue(res_ting_hu, type * 100 + 2, tempHuxi + tempTypeHuxi[type],headcombi)
    ...
    pengHuTingpaiLogic.setTing_huValue(res_ting_hu, handpokers[1], tempHuxi + tempTypeHuxi[type],headcombi)
    if (type == 1) then
        pengHuTingpaiLogic.setTing_huValue(res_ting_hu, handpokers[1] + 100, tempHuxi,headcombi)
    else
        pengHuTingpaiLogic.setTing_huValue(res_ting_hu, handpokers[1] - 100, tempHuxi,headcombi)
    end
    local res = pengHuTingpaiLogic.newTingPaiResStruct()
...
function pengHuTingpaiLogic.STwoKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
    local handSize = #handpokers
    local tempTypeHuxi = { g_phzHuxi.s_xiao,g_phzHuxi.b_xiao}
    local res = pengHuTingpaiLogic.newTingPaiResStruct()
    if handSize == 0 then
        ...
        for i,paiValue in ipairs(pengHuTingpaiLogic.AllPaiValue) do
            local type = pengHuTingpaiLogic.getPaiType(paiValue)
            headcombi[#headcombi][1] = paiValue
    ...
    elseif handSize == 3 then
		...
		for i,comb in ipairs(indexValue) do
			...
			if (size > 0) then
				...
				local tempRet = pengHuTingpaiLogic.SOneKindPaiTingRes(temp, res_ting_hu, tempHuxi + maxTempHuxi,headcombi)
                pengHuTingpaiLogic.updateTingPaiResStruct(res,tempRet)
            end
		end
    end
...
function pengHuTingpaiLogic.SThreeKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
    local tempTypeHuxi = { g_phzHuxi.s_xiao,g_phzHuxi.b_xiao}
    ...
    for  i = 1, #handpokers do
        type[#type + 1] = pengHuTingpaiLogic.getPaiType(handpokers[i])
        value[#value + 1] = pengHuTingpaiLogic.getPaiType(handpokers[i])
    end
    if handSise == 2 then
        headcombi[#headcombi + 1] = {}
        headcombi[#headcombi + 1] = {}
		...
		if (size > 0) then
			...
			for i,paiValue in ipairs (pengHuTingpaiLogic.AllPaiValue) do
				local temptype = pengHuTingpaiLogic.getPaiType(paiValue)
                headcombi[#headcombi] = {paiValue,paiValue}
                pengHuTingpaiLogic.setTing_huValue(res_ting_hu, paiValue, tempHuxi + maxTempHuxi + tempTypeHuxi[temptype],headcombi)
            end
		end
        table.remove(headcombi,#headcombi)
		...
		pengHuTingpaiLogic.SOneKindPaiTingRes(tempHand1, res_ting_hu, tempHuxi + tempTypeHuxi[type[1]],headcombi)
        headcombi[#headcombi] = {handpokers[2],handpokers[2],handpokers[2]}
		pengHuTingpaiLogic.SOneKindPaiTingRes(tempHand2, res_ting_hu, tempHuxi + tempTypeHuxi[type[2]],headcombi)
    ...
    return pengHuTingpaiLogic.newTingPaiResStruct()
end

function pengHuTingpaiLogic.SFourKindPaiTingRes(handpokers, res_ting_hu, tempHuxi,headcombi)
    --四张王 剩 1 4 7
    ...
    if handSise == 1 then
        ...
        for i,paiValue in ipairs(pengHuTingpaiLogic.AllPaiValue) do
            pengHuTingpaiLogic.SOneKindPaiTingRes({paiValue},res_ting_hu,tempHuxi + g_phzHuxi.b_xiao,headcombi)
        end
        headcombi[#headcombi] = {handpokers[1],handpokers[1]}
        headcombi[#headcombi + 1] = {0,0,0}

        for i,paiValue in ipairs(pengHuTingpaiLogic.AllPaiValue) do            --这里可以提胡所有牌
            headcombi[#headcombi][1] = paiValue
            headcombi[#headcombi][2] = paiValue
    ...
    elseif handSise == 4 then
		...
		elseif (#markTempl >= kindNum - 1) then
            tempHandPoker[#tempHandPoker + 1] = g_phzCards.kind_CardValue
			return pengHuTingpaiLogic.markDnfTingCard(markTempl, tempHandPoker, res_ting_hu, kindNum - 1, tempHuxi,headcombi)
        end
    elseif handSise == 7 then
		...
		elseif (#markTempl >= kindNum - 2) then
            tempHandPoker[#tempHandPoker + 1] = g_phzCards.kind_CardValue
            tempHandPoker[#tempHandPoker + 1] = g_phzCards.kind_CardValue
			return pengHuTingpaiLogic.markDnfTingCard(markTempl, tempHandPoker, res_ting_hu, kindNum - 2, tempHuxi,headcombi)
        end
    end
    return pengHuTingpaiLogic.newTingPaiResStruct()
end

function pengHuTingpaiLogic.markDnfTingCard(markTempl,tempHandPoker,res_ting_hu,kindNum,tempHuxi,headcombi)
    local kindCombis = pengHuTingpaiLogic.combinationMarkTempList(markTempl, kindNum)

    local tingRet = pengHuTingpaiLogic.newTingPaiResStruct()
    for k = 1, kindNum do
	...
	for j = 1, #kindCombis do
        ...
        local tempRet = pengHuTingpaiLogic.getTingPaiList(tempHandPoker, res_ting_hu, tempHuxi,headcombi)
        pengHuTingpaiLogic.updateTingPaiResStruct(tingRet,tempRet)
	end
    return tingRet
end

function pengHuTingpaiLogic.replaceKindPaiDnfTingpai(handpokers,res_ting_hu,kindNum,tempHuxi,headcombi)
    local markTempList = pengHuTingpaiLogic.getmarkTempl(handpokers)
    local tempHandPoker = table.clone(handpokers)
    local tingRet = pengHuTingpaiLogic.newTingPaiResStruct()
    local kindCombis = {}
    if #markTempList >= kindNum then
        kindCombis = pengHuTingpaiLogic.combinationMarkTempList(markTempList, kindNum)
    end
...
function pengHuTingpaiLogic.getmarkTempl(handpokers)
    local markTempl = {}
	local pai_count = {}
	...
	local count = 0
	for i = 1,#handpokers do
		type = pengHuTingpaiLogic.getPaiType(handpokers[i])
		value = pengHuTingpaiLogic.getPaiValue(handpokers[i])
...
function pengHuTingpaiLogic.getCountAndDelByHand(handpokers, paiValue)
    local res = 0
...
function pengHuTingpaiLogic.getxiaoQingHuxi(xiaoQingPai)
    local res = 0
	local type = 0
	local size = 0
	for  i = 1, #xiaoQingPai do
	
		type = pengHuTingpaiLogic.getPaiType(xiaoQingPai[i][1])
		size = #xiaoQingPai[i]
		if (type == 1) then--小
...
function pengHuTingpaiLogic.getchuTingPairByTwo(combi,resChu_ting,pubicHuxi,headcombi)   --没跑起的情况下，手里会省2张牌做门子
    local comSize = #combi
    local type = {}
    local value = {}
    table.sort(combi)
    for i,v in ipairs(combi) do
        type[i] = pengHuTingpaiLogic.getPaiType(combi[i])
		value[i] = pengHuTingpaiLogic.getPaiValue(combi[i])
    end
    return pengHuTingpaiLogic.setChuTingCard(combi, resChu_ting, value, type, 1, 2, pubicHuxi,headcombi)
end

function pengHuTingpaiLogic.getchuTingPairByOne(combi,resChu_ting,pubicHuxi,headcombi)
    headcombi[#headcombi + 1] = combi
    pengHuTingpaiLogic.setTing_huValue(resChu_ting,combi[1],pubicHuxi,headcombi)
    return true
end

function pengHuTingpaiLogic.getchuTingPairByFour(combi,resChu_ting,pubicHuxi,headcombi) --跑起来了，要一对将牌
    local comSize = #combi
    local type = {}
    local value = {}
	...
	if (type[1] == type[2] and value[1] == value[2]) then
        ...
        local tempisTing = pengHuTingpaiLogic.setChuTingCard(combi, resChu_ting, value, type, 3, 4, pubicHuxi,headcombi)
	...
	if type[3] == type[4] and value[3] == value[4] then
        ...
        local tempisTing = pengHuTingpaiLogic.setChuTingCard(combi, resChu_ting, value, type, 1, 2, pubicHuxi,headcombi)
        isTing = isTing or tempisTing
    end
	...
	for  i = 1, #indexValue do
		...
		if (pengHuTingpaiLogic.isVaildCombi({ combi[indexValue[i][1]],combi[indexValue[i][2]], combi[indexValue[i][3]] })) then
            ...
            pengHuTingpaiLogic.setTing_huValue(resChu_ting,combi[firstIndex],pubicHuxi + tempHufen,headcombi)
            isTing = true
		end
    end
    return isTing
...
-- function pengHuTingpaiLogic.setTing_huValue(resChu_ting,ting,huxi,headcombi)
--     if resChu_ting[ting] == nil then
--         resChu_ting[ting] = {}
--         resChu_ting[ting][pengHuTingpaiLogic.resIndex.huxi] = huxi
--         resChu_ting[ting][pengHuTingpaiLogic.resIndex.combi] = {}
--         resChu_ting[ting][pengHuTingpaiLogic.resIndex.combi] = table.clone(headcombi)
--     end

--     if huxi > resChu_ting[ting][pengHuTingpaiLogic.resIndex.huxi] then
--         resChu_ting[ting][pengHuTingpaiLogic.resIndex.huxi] = huxi
--         resChu_ting[ting][pengHuTingpaiLogic.resIndex.combi] = table.clone(headcombi)
--     end
-- end
...
function pengHuTingpaiLogic.setTing_huValue(resChu_ting,ting,huxi,headcombi)
    local info = {}
    info[3] = ting
    info[pengHuTingpaiLogic.resIndex.huxi] = huxi
    info[pengHuTingpaiLogic.resIndex.combi] = {}
    info[pengHuTingpaiLogic.resIndex.combi] = table.clone(headcombi)
...
function pengHuTingpaiLogic.setChuTingCard(combi, resChu_ting, value, type, firstIndex, scondIndex, pubicHuxi,headcombi)
    local tempNewHeadCombi = {}
    local isTing = false
		...
		if (value[firstIndex] ~= 1) then
			if (value[firstIndex] - 1 == 1) then                   --听的是1
                if (type[firstIndex] == 1) then
                    pengHuTingpaiLogic.setTing_huValue(resChu_ting,combi[firstIndex] - 1,pubicHuxi + g_phzHuxi.s_chi,tempNewHeadCombi)
                    isTing = true
                else
                    pengHuTingpaiLogic.setTing_huValue(resChu_ting,combi[firstIndex] - 1,pubicHuxi + g_phzHuxi.b_chi,tempNewHeadCombi)
                    isTing = true
                end
            else
                pengHuTingpaiLogic.setTing_huValue(resChu_ting,combi[firstIndex] - 1,pubicHuxi,tempNewHeadCombi)
                 isTing = true
            end
        end
		...
		if (value[scondIndex] ~= 10) then
			...
			if (value[scondIndex] + 1 == 3) then           --听的是3
				if (type[scondIndex] == 1) then
                    pengHuTingpaiLogic.setTing_huValue(resChu_ting,combi[scondIndex] + 1,pubicHuxi + g_phzHuxi.s_chi,tempNewHeadCombi)
                    isTing = true
                else
                    pengHuTingpaiLogic.setTing_huValue(resChu_ting,combi[scondIndex] + 1,pubicHuxi + g_phzHuxi.b_chi,tempNewHeadCombi)
                    isTing = true
                end
            else
                pengHuTingpaiLogic.setTing_huValue(resChu_ting,combi[scondIndex] + 1,pubicHuxi,tempNewHeadCombi)
                isTing = true
            end
        end
	...
	elseif (value[firstIndex] == value[scondIndex]) then
        pengHuTingpaiLogic.setTing_huValue(resChu_ting,value[firstIndex] + 100,pubicHuxi,tempNewHeadCombi)
        pengHuTingpaiLogic.setTing_huValue(resChu_ting,value[firstIndex] + 200,pubicHuxi,tempNewHeadCombi)
        isTing = true
        if type[firstIndex] == type[scondIndex] then
            if type[scondIndex] == 2 then 
                pengHuTingpaiLogic.setTing_huValue(resChu_ting,value[firstIndex] + 200,pubicHuxi + g_phzHuxi.b_xiao ,tempNewHeadCombi)
            else        
                pengHuTingpaiLogic.setTing_huValue(resChu_ting,value[firstIndex] + 100,pubicHuxi + g_phzHuxi.s_xiao,tempNewHeadCombi)
            end 
        end
        ...
        if (type[firstIndex] == type[scondIndex]) then
			...
			if (value[firstIndex] == 2 and value[scondIndex] == 7) then
                if (type[firstIndex] == 1) then
                    pengHuTingpaiLogic.setTing_huValue(resChu_ting,combi[scondIndex] + 3,pubicHuxi + g_phzHuxi.s_chi,tempNewHeadCombi)
                    isTing = true
                else
                    pengHuTingpaiLogic.setTing_huValue(resChu_ting,combi[scondIndex] + 3,pubicHuxi + g_phzHuxi.b_chi,tempNewHeadCombi)
                    isTing = true
                end
			...
			elseif (value[firstIndex] == 2 and value[scondIndex] == 10) then
				...
				if (type[firstIndex] == 1) then
                    pengHuTingpaiLogic.setTing_huValue(resChu_ting,combi[firstIndex] + 5,pubicHuxi + g_phzHuxi.s_chi,tempNewHeadCombi)
                    isTing = true
				else
                    pengHuTingpaiLogic.setTing_huValue(resChu_ting,combi[firstIndex] + 5,pubicHuxi + g_phzHuxi.b_chi,tempNewHeadCombi)
                    isTing = true
                end
...
function pengHuTingpaiLogic.GetvalidCombiHufen(combi) 
    local res = 0
	local types = {}
	local values = {}
	for  i = 1, #combi - 1 do              --最后一组不是有效组合
		...
		for  LOOP = 1, #combi[i] do
			types[LOOP] = pengHuTingpaiLogic.getPaiType(combi[i][LOOP])
			values[LOOP] = pengHuTingpaiLogic.getPaiValue(combi[i][LOOP])
        end
...
function pengHuTingpaiLogic.resolvexiaoQingPai(_handpokers)
   local res = {}
...
function pengHuTingpaiLogic.getValidComByHandpokers(handpokers)
	local pai_count = {}
	local retComs = {}
	...
	for i,paiValue in ipairs(handpokers) do
		if prePaivalue ~= paiValue then
            ...
            if (pengHuTingpaiLogic.getPaiValue(paiValue) == 2) then     --二七十
            ...
            if (pai_count[paiValue] == 2) then
                ...
                local type = pengHuTingpaiLogic.getPaiType(paiValue);         --壹壹一
                if (type == 1) then
...
function pengHuTingpaiLogic.getAllCardCombi(handPokers,kindNum)
    ...
    local vaildcombis = pengHuTingpaiLogic.getValidComByHandpokers(handPokers)--pengHuTingpaiLogic.combinationZhuhe(handPokers,3,pengHuTingpaiLogic.isVaildCombi)
    ...
    for i = tempComSize, comsize, -1 do
        if #vaildcombis >= i then
            pengHuTingpaiLogic.combinationZhuheList(pai_count,vaildcombis, i,tempcombss)
        end
    end

    for i,combis in ipairs(tempcombss) do
        local tempMenzi = pengHuTingpaiLogic.getMenzi(combis,handPokers)
...
function pengHuTingpaiLogic.getMenzi(combis,handpokers)
    local pai_Index = {}
    for i = 1,#handpokers do
...
function pengHuTingpaiLogic.getPaiType(paiNum)
    return math.floor( paiNum / 100 )
end

function pengHuTingpaiLogic.getPaiValue(paiNum)
    return paiNum % 100
end

function pengHuTingpaiLogic.isVaildCombi(combi)
    local type = {0,0,0}
	local value = {0,0,0}
...
function pengHuTingpaiLogic.combinationZhuheList(pai_Count,sumList, nComLen,retList) --获取有效手牌路径
...
function pengHuTingpaiLogic.combinationMarkTempList(sumList, nComLen)
    local retList = {}
    nComLen = nComLen > #sumList and #sumList or nComLen
	if nComLen == 0 then
		return retList
