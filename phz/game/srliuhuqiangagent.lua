...
local deskInfo = 
{
	...
	smallState = 0,
	bigState = 0,
	gameid = PDEFINE.GAME_TYPE.ZIPAI_LIUHUQIANG_SR
}
...
local function big_over()
	for _,user in pairs(deskInfo.users) do
		pcall(cluster.call, user.cluster_info.server, user.cluster_info.address, "deskBack", PDEFINE.GAME_TYPE.ZIPAI_LIUHUQIANG_SR) --释放桌子对象
	end
	deskInfo.users = {}
	deskInfo.smallState = 0
	deskInfo.bigState = 0
...
function CMD.startGame() --2种开始方式  庄家设置也不一样 
	...
	notify_retobj.code   = PDEFINE.RET.SUCCESS
    notify_retobj.gameid = PDEFINE.GAME_TYPE.ZIPAI_LIUHUQIANG_SR
