...
local deskInfo = 
{
	...
	smallState = 0,
	bigState = 0,
	gameid = PDEFINE.GAME_TYPE.ZIPAI_PAOHZ_SR
}
...
local function big_over()
	for _,user in pairs(deskInfo.users) do
		pcall(cluster.call, user.cluster_info.server, user.cluster_info.address, "deskBack", PDEFINE.GAME_TYPE.ZIPAI_PAOHZ_SR) --释放桌子对象
	end
	deskInfo.users = {}
	deskInfo.smallState = 0
	deskInfo.bigState = 0
...
local function checkPengGangHu(card,uid,putType)
	...
	if putType == paohuzitool.lpType.draw then
		...
		local ok,huXi,huCards = paohuzitool.checkPaoHuZiHu(ownuser,checkHuCards,card,user.uid)
		if huXi then
			-- if not ok then
		...
		local ok,huxi,huCards = paohuzitool.checkPaoHuZiHu(nextUser,checkHuCards,card,user.uid)
		if huxi then
...
local function draw(seat)
	...
	if ret then
	    ...
	    local dactionInfo = table.copy(deskInfo.actionInfo)
...
function CMD.startGame() --2种开始方式  庄家设置也不一样 
	...
	notify_retobj.code   = PDEFINE.RET.SUCCESS
    notify_retobj.gameid = PDEFINE.GAME_TYPE.ZIPAI_PAOHZ_SR
	...
	--         [21] = 202
 --    	}
	-- 	  usersCard[2] = {205,205,205,109,109,209,209,201,201,203,203,108}
	-- else
	-- 	usersCard[1] =  {205,205,205,109,109,209,209,201,201,203,203,108}
	-- 	usersCard[2] = {
	-- 		[1] = 110,
	--         [2] = 110,
	--         [3] = 210,
