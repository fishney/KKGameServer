...
local deskInfo = 
{
	...
	smallState = 0,
	bigState = 0,
	gameid = PDEFINE.GAME_TYPE.ZIPAI_PENGHZ_SR
}
...
local function initBankerInfo()
	if deskInfo.bankerInfo.uid == 0 then
		...
		deskInfo.bankerInfo.realyZjCount = 0
...
local function big_over()
	for _,user in pairs(deskInfo.users) do
		pcall(cluster.call, user.cluster_info.server, user.cluster_info.address, "deskBack", PDEFINE.GAME_TYPE.ZIPAI_PENGHZ_SR) --释放桌子对象
	end
	deskInfo.users = {}
	deskInfo.smallState = 0
	deskInfo.bigState = 0
	deskInfo.bankerInfo = {uid = 0, count = 1, seat = 0,realyZjCount = 0}
	deskInfo.actionInfo = {iswait = 0,prioritySeat = 0, waitList = {},curaction = {seat = 0, type = 0, card = 0, source = 0},nextaction = {seat = 0, type = 0}}
		...
		if baseScore then
			if hupaiType == penghutool.HUPAI_TYPE.tianHu or hupaiType == penghutool.HUPAI_TYPE.shuangLongHu or hupaiType == penghutool.HUPAI_TYPE.xiaoQiDuiHu then
				...
				if not dianPaoUser then
					...
					for _,user in pairs(deskInfo.users) do
						if user.seat ~= hseat then
							user.changeScore = -baseScore-(4*(deskInfo.bankerInfo.count-2))
...
local function getChekHuPaiList(user)
	local checkHuCards = {}
	...
	if ret then
		...
		if ret == penghutool.cardType.kan then
			...
			for _, menziInfo in pairs(duser.menzi) do
				if menziInfo.type == penghutool.cardType.peng or menziInfo.type == penghutool.cardType.kan or menziInfo.type == penghutool.cardType.long or menziInfo.type == penghutool.cardType.she then
		...
		elseif ret == penghutool.cardType.hupai then
			local dianPaoSeat
			if huPaiType == penghutool.HUPAI_TYPE.pengHu then
				local scoeTypeCnt = 0
				for _, menziInfo in pairs(duser.menzi) do
					if menziInfo.type == penghutool.cardType.peng or menziInfo.type == penghutool.cardType.kan or menziInfo.type == penghutool.cardType.long or menziInfo.type == penghutool.cardType.she then
						scoeTypeCnt = scoeTypeCnt + 1
				...
				if scoeTypeCnt == 4 then
					if duser.isBaoJin then
						huPaiType = penghutool.HUPAI_TYPE.wuHu
					else
						huPaiType = penghutool.HUPAI_TYPE.pengSiHu
					end
				end
				if deskInfo.actionInfo.curaction.seat == seat then
					if huPaiType == penghutool.HUPAI_TYPE.pengSanHu then
						huPaiType = penghutool.HUPAI_TYPE.saoSanHu
					elseif huPaiType == penghutool.HUPAI_TYPE.pengSiHu then
			...
			if huPaiType == penghutool.HUPAI_TYPE.paoHu then
				if deskInfo.actionInfo.curaction.seat == seat then
					for _, menziInfo in pairs(user.menzi) do
						if menziInfo.type == penghutool.cardType.kan and  menziInfo.card == drawCard then
							huPaiType = penghutool.HUPAI_TYPE.tilongHu
							break
						end
					end
				end
...
function CMD.peng(source,msg)
	...
	if penghutool.getValueCount(user.handInCards,pcard) == 2 then
		...
		local scoeTypeCnt = 0
		for _, menziInfo in pairs(user.menzi) do
			if menziInfo.type == penghutool.cardType.peng or menziInfo.type == penghutool.cardType.kan or menziInfo.type == penghutool.cardType.long or menziInfo.type == penghutool.cardType.she or menziInfo.type == penghutool.cardType.pao then
				scoeTypeCnt = scoeTypeCnt + 1
			end
		end
		if scoeTypeCnt == 3 then
...
function CMD.pass(source,msg)
	...
	-- if deskInfo.actionInfo.iswait == 0 then
	-- 	local nextSeat = getNextSeat(deskInfo.actionInfo.curaction.seat)
	-- 	local cuser = seleteUserInfo(uid,"uid")
	-- 	table.insert(cuser.qipai,deskInfo.actionInfo.curaction.card)
	-- 	deskInfo.actionInfo.nextaction = {seat = nextSeat,type = penghutool.cardType.draw}
	-- 	deskInfo.actionInfo.iswait = 0
	-- 	deskInfo.actionInfo.waitList = {}
	-- 	local noty_retobj  = {}
...
function CMD.startGame() --2种开始方式  庄家设置也不一样 
	...
	notify_retobj.code   = PDEFINE.RET.SUCCESS
    notify_retobj.gameid = PDEFINE.GAME_TYPE.ZIPAI_PENGHZ_SR
	...
	-- if #usersCard[1] == 15 then
	-- 	usersCard[1] = {105,105,202,207,210,205,205,107,108,109,207,208,209,206,101}
	-- 	usersCard[2] = {105,101,101,206,207,209,201,201,201,203,203,203}
	-- 	usersCard[3] = {105,101,101,206,207,209,201,201,201,203,203,203}
	-- 	usersCard[4] = {105,101,101,206,207,209,201,201,201,203,203,203}
	-- else
	-- 	usersCard[1] =  {105,101,101,206,207,209,201,201,201,203,203,203}
	-- 	usersCard[2] = {105,105,202,207,210,205,205,107,108,109,207,208,209,206,101}
	-- 	usersCard[3] = {105,101,101,206,207,209,201,201,201,203,203,203}
	-- 	usersCard[4] = {105,101,101,206,207,209,201,201,201,203,203,203}
	-- end
    --new_card = {105,210,105}
 -- 	if #usersCard[1] == 15 then
	-- 	usersCard[1] = {109,109,209,208,108,108,110,110,210,105,105,205,201,202,203}
	-- 	usersCard[2] = {203,203,107,209,210,110,104,204,202,201}
	-- else
	-- 	usersCard[2] = {203,203,107,209,210,110,104,204,202,201}
	-- 	usersCard[1] = {109,109,209,208,108,108,110,110,210,105,105,205,201,202,203}
	-- end
 --    new_card = {109,210,205,201,205,101,101,109}
...
-- 加入桌子
function CMD.hallJoin(source,uid,cluster_info,ip,lat,lng)
	local user = seleteUserInfo(uid,"uid")
	if user then
