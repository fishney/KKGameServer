...
local deskInfo = 
{
	...
	smallState = 0,
	bigState = 0,
	gameid = PDEFINE.GAME_TYPE.ZIPAI_HONGHEIHU
}
...
local function big_over()
	for _,user in pairs(deskInfo.users) do
		pcall(cluster.call, user.cluster_info.server, user.cluster_info.address, "deskBack", PDEFINE.GAME_TYPE.ZIPAI_HONGHEIHU) --释放桌子对象
...
local function getCard(buck) --庄家多抓一张
	...
	for i,cards in pairs(userscard) do
		local varNum = getCardCount(cards)
		local sheCount = 0
		for value ,count in pairs(varNum) do
			if count == 4 then
				sheCount = sheCount + 1
			end
		end
...
function CMD.startGame() --2种开始方式  庄家设置也不一样 
	...
	notify_retobj.code   = PDEFINE.RET.SUCCESS
    notify_retobj.gameid = PDEFINE.GAME_TYPE.ZIPAI_HONGHEIHU
	...
	--         [17] = 207,
 --    	}
	-- 	usersCard[1] = {105,101,206,206,207,207,201,201,201,205,208,209}
	--  else
	--  	usersCard[1] =  {
	--  		[1] = 105,
	--         [2] = 105,
	...
	--         [17] = 207,
 --    	}
	--  	usersCard[2] = {105,101,206,206,207,207,201,201,201,205,208,209}
	--  end
 --     new_card = {209,108,210,210,210}
