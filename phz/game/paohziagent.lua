...
local paohuzitool = require "paohuzitool"
...
local deskInfo = 
{
	...
	smallState = 0,
	bigState = 0,
	gameid = PDEFINE.GAME_TYPE.ZIPAI_PAOHZ
}
...
local function getActionInfo()
	...
	deskInfo.actionInfo.nextaction.type =  paohuzitool.cardType.put
end
...
local function notyTingPaiInfo(user)
	...
	notify_retobj.tingPaiInfo = paohuzitool.getTingPaiInfo(user)
...
local function getOherMemzi(duser)
	...
	for _,info in pairs(duser.menzi) do
		if info.type == paohuzitool.cardType.kan or info.type == paohuzitool.cardType.long then
			local otherInfo = {}
...
local function notyGpsColour()
	deskInfo.locatingList,gpsColour = paohuzitool.jisuanXY(deskInfo.users)
	local noty_retobj = {}
...
local function big_over()
	for _,user in pairs(deskInfo.users) do
		pcall(cluster.call, user.cluster_info.server, user.cluster_info.address, "deskBack", PDEFINE.GAME_TYPE.ZIPAI_PAOHZ) --释放桌子对象
...
local function getMenziPaoOrlongOrShe(user)
	for _,info in pairs(user.menzi) do
		if info.type >= paohuzitool.cardType.pao then
			return true
		end
	end
	return paohuzitool.checkHandShe(user.handInCards) ---校验手中是否有蛇
end

local function hupaiBance(hseat,hcard,huXi,isdissolve,huCards)
	closeAllTimer()
	...
	if huXi > 0 then
		...
		local roundScore = paohuzitool.checkScore(huXi,deskInfo.conf.param1,deskInfo.conf.score)
		for _, muser in pairs(deskInfo.users) do
		...
		noty_retobj.hupaiType = paohuzitool.HUPAI_TYPE.normal
		gameRecord(1)
	else--流局
...
local function getChekHuPaiList(user,types) --把下面的牌都放到牌堆里算胡牌
	...
	-- 		table.insert(checkHuCards,info.card)
	-- 	end
	-- 	if info.type == paohuzitool.cardType.long then
	-- 		table.insert(checkHuCards,info.card)
	-- 		table.insert(checkHuCards,info.card)
	-- 		table.insert(checkHuCards,info.card)
	-- 		table.insert(checkHuCards,info.card)
...
local function checkPengGangHu(card,uid,putType)
	...
	--校验自己
	if putType == paohuzitool.lpType.draw then
		local checkHuCards = getChekHuPaiList(ownuser)
		local ok,huXi,huCards = paohuzitool.checkPaoHuZiHu(ownuser,checkHuCards,card,uid)
		if huXi then
			-- if not ok then
			-- 	if paohuzitool.getValueCount(checkHuCards,card) == 2 then
			-- 		return paohuzitool.cardType.kan,ownuser.seat
			-- 	end
			-- 	return paohuzitool.cardType.hupai,ownuser.seat,huXi,huCards
			-- else
			-- 	return paohuzitool.cardType.hupai,ownuser.seat,huXi,huCards
			-- end
			return paohuzitool.cardType.hupai,ownuser.seat,huXi,huCards
		...
		--自己摸得牌,
		--1.校验自己是不是踢龙,
		local ok,tiLongType = paohuzitool.checkTiLong(ownuser,card)
		if ok then
			return tiLongType,ownuser.seat
		end

		--2.校验自己是不是喂牌
		if paohuzitool.checkSao(ownuser,card) then
			return paohuzitool.cardType.kan,ownuser.seat
		end
		...
		local ok,huxi,huCards = paohuzitool.checkPaoHuZiHu(nextUser,checkHuCards,card,uid)
		if huxi then
			return paohuzitool.cardType.hupai,nextUser.seat,huxi,huCards
		end
		...
		--1.校验自己是不是碰跑
		if paohuzitool.checkPengPao(ownuser,card) then
			return paohuzitool.cardType.pao,ownuser.seat
		end

		--1.校验别人是不是跑
		for _, user in pairs(deskInfo.users) do
			if user.uid ~= uid then
				if paohuzitool.checkPengPao(user,card) then
					return paohuzitool.cardType.pao,user.seat
				end
				local ok, paoType = paohuzitool.checkKanPao(user,card)
				if ok then
		...
		for _, user in pairs(deskInfo.users) do
			if user.uid ~= uid then
				if paohuzitool.checkPeng(user,card) then
					...
					if deskInfo.actionInfo.waitList[user.seat] == nil then
						...
						local info = {}
						info.type = paohuzitool.cardType.guo
						info.seat = user.seat
					...
					info.level = 3
					info.seat = user.seat
					info.type = paohuzitool.cardType.peng
					info.data = card
					table.insert(deskInfo.actionInfo.waitList[user.seat],info)
				end
			end
		...
		local chiTypeList = paohuzitool.checkChi(ownuser,card,sUser)
		local ownIschi
		if chiTypeList then
			deskInfo.actionInfo.iswait = 1
			...
			info.seat = ownuser.seat
			info.level = 2
			info.type = paohuzitool.cardType.chi
		...
		local sUser = seleteUserInfo(sSeat,"seat")
		local chiTypeList = paohuzitool.checkChi(nextUser,card,sUser)
		if chiTypeList then
			deskInfo.actionInfo.iswait = 1
...
local function draw(seat)
	...
	if not drawCard then
		hupaiBance(deskInfo.actionInfo.curaction.seat, deskInfo.actionInfo.curaction.card, paohuzitool.HUPAI_TYPE.liuju)
		return
	end
	...
	local ret,dseat,huXi,huCards = checkPengGangHu(drawCard,user.uid,paohuzitool.lpType.draw)
	if ret then
		if ret == paohuzitool.cardType.kan or ret == paohuzitool.cardType.long or ret == paohuzitool.cardType.handlong then
			for _, muser in pairs(deskInfo.users) do
	...
	deskInfo.actionInfo.curaction = {seat = user.seat, type = paohuzitool.cardType.draw, time = timeout[1], card = drawCard, source = 0}
	...
	if ret then
		...
		local noty_retobj  = {}
		if ret == paohuzitool.cardType.kan then
			delteHandIncards(duser,drawCard)
			delteHandIncards(duser,drawCard)
			local kanType = paohuzitool.cardType.kan
			local info = {}
			info.type = paohuzitool.cardType.kan
			info.card = drawCard
			table.insert(duser.menzi,info)

			local isJiu
			if paohuzitool.findCard(duser.notPengPai,drawCard) then
				isJiu = drawCard
			end

			noty_retobj.c      = PDEFINE.NOTIFY.NOTIFY_HZ_KAN
			noty_retobj.kanType = kanType
			noty_retobj.isJiu   = isJiu
			deskInfo.actionInfo.nextaction = {seat = duser.seat,type = paohuzitool.cardType.put}
			CMD.userSetAutoState("autoPut",timeout[1]*100,deskInfo.actionInfo.nextaction.seat)
		elseif ret == paohuzitool.cardType.long then
			...
			if isChongPao then
				...
				deskInfo.actionInfo.nextaction = {seat = nextSeat,type = paohuzitool.cardType.draw}
			else
				deskInfo.actionInfo.nextaction = {seat = duser.seat,type = paohuzitool.cardType.put}
				CMD.userSetAutoState("autoPut",timeout[1]*100,deskInfo.actionInfo.nextaction.seat)
			end
			...
			for i, info in pairs(duser.menzi) do
				if info.type == paohuzitool.cardType.kan then
					duser.menzi[i].type = paohuzitool.cardType.long
					break
				end
			end
		elseif ret == paohuzitool.cardType.handlong then
			noty_retobj.c      = PDEFINE.NOTIFY.NOTIFY_HZ_TILONG
			...
			local info = {}
			info.type = paohuzitool.cardType.long
			info.card = drawCard
			table.insert(duser.menzi,info)
			delteHandIncards(duser,drawCard)
			delteHandIncards(duser,drawCard)
		...
		elseif ret == paohuzitool.cardType.pao then
			...
			for i, info in pairs(duser.menzi) do
				if info.type == paohuzitool.cardType.kan or info.type == paohuzitool.cardType.peng then
					if info.card == drawCard then
						duser.menzi[i].type = paohuzitool.cardType.pao
						break
					end
				end
			end
		elseif ret == paohuzitool.cardType.handpao then
			noty_retobj.c      = PDEFINE.NOTIFY.NOTIFY_HZ_PAO
			...
			local info = {}
			info.type = paohuzitool.cardType.pao
			info.card = drawCard
			table.insert(duser.menzi,info)
			delteHandIncards(duser,drawCard)
			delteHandIncards(duser,drawCard)
		...
		elseif ret == paohuzitool.cardType.hupai then
			hupaiBance(dseat,drawCard,huXi,nil,huCards)
			return 
		end
	 ...
	 --    	local kanCard = deskInfo.actionInfo.curaction.card
	    ...
	    noty_retobj.gameid = deskInfo.gameid
	    noty_retobj.seat   = user.seat
	    deskInfo.actionInfo.curaction = {seat = user.seat, type = paohuzitool.cardType.put,time = timeout[1], card = drawCard, source = 0}
	...
	if table.size(deskInfo.actionInfo.waitList) == 1 then
		...
		for dst, waitUsers in pairs(deskInfo.actionInfo.waitList) do
			for _, waitInfo in pairs(waitUsers) do
				if waitInfo.type == paohuzitool.cardType.peng then
					isPeng = dst
					break
				end
			end
...
local function autoPut(seat)
	local user = seleteUserInfo(seat,"seat")
	--判断打牌者是否这个用户
	if deskInfo.actionInfo.nextaction.seat ~= user.seat or deskInfo.actionInfo.nextaction.type ~= paohuzitool.cardType.put then
	...
	if card then
		...
		deskInfo.actionInfo.curaction.type = paohuzitool.cardType.put
	...
	-- 通知其它玩家打牌
	local ret,dseat = checkPengGangHu(card,user.uid,paohuzitool.lpType.put)
	if ret then
		local noty_retobj  = {}
	    noty_retobj.c      = PDEFINE.NOTIFY.NOTIFY_HZ_PUT
	    noty_retobj.code   = PDEFINE.RET.SUCCESS
		...
		local duser = seleteUserInfo(dseat,"seat")
		local noty_retobj  = {}
		if ret == paohuzitool.cardType.pao or ret == paohuzitool.cardType.handpao then
			noty_retobj.c      = PDEFINE.NOTIFY.NOTIFY_HZ_PAO
			isChongPao = getMenziPaoOrlongOrShe(duser)
			if ret == paohuzitool.cardType.pao then
				noty_retobj.ishand = 0 --是不是手上的
				for i, info in pairs(duser.menzi) do
					if info.type == paohuzitool.cardType.kan or info.type == paohuzitool.cardType.peng then
	    ...
	    if deskInfo.actionInfo.iswait == 1 then
			...
			local nextSeat = getNextSeat(user.seat)
			deskInfo.actionInfo.nextaction = {seat = nextSeat,type = paohuzitool.cardType.draw}
			deskInfo.actionInfo.curaction = {seat = user.seat, type = paohuzitool.cardType.put, time = timeout[1], card = card, source = user.seat}
			deskInfo.actionInfo.iswait = 0
			deskInfo.actionInfo.waitList = {}
			noty_retobj.actionInfo = deskInfo.actionInfo
	    	broadcastDesk(cjson.encode(noty_retobj))
...
local function autoPass(seat)
	...
	--     broadcastDesk(cjson.encode(noty_retobj))
	--     if #new_card == 0 then
	--     	hupaiBance(-1,-1,paohuzitool.HUPAI_TYPE.liuju)
	-- 		return PDEFINE.RET.SUCCESS
	-- 	end
	--     draw(nextSeat)
	-- else
...
function CMD.peng(source,msg)
	...
	--local pengType = paohuzitool.cardType.peng
	local pengInfo = {}
	if paohuzitool.getValueCount(user.handInCards,pcard) == 2 then
		local info = {}
		info.type = paohuzitool.cardType.peng
		info.card = pcard
		...
		delteHandIncards(user,pcard)
		deskInfo.actionInfo.curaction = {seat = user.seat, type = paohuzitool.cardType.peng, time = timeout[1], card = deskInfo.actionInfo.curaction.card, source = deskInfo.actionInfo.curaction.source}
		deskInfo.actionInfo.nextaction = {seat = user.seat,type = paohuzitool.cardType.put}
		deskInfo.actionInfo.iswait = 0
		deskInfo.actionInfo.waitList = {}
		local noty_retobj  = {}
	    ...
	    noty_retobj.gameid = deskInfo.gameid
	    noty_retobj.seat   = user.seat
	    noty_retobj.pengType = paohuzitool.cardType.peng
	    noty_retobj.actionInfo = deskInfo.actionInfo
	    paohuzitool.setHuXi(user)
	    for _, muser in pairs(deskInfo.users) do
...
function CMD.chiPai(source,msg,isLocal)
	...
	if compWaitList(user.seat,msg) then
		for _, waitInfo in pairs(waitList) do
			if waitInfo.type == paohuzitool.cardType.chi then
		...
		local infoData = {}
		infoData.type = paohuzitool.cardType.chi
		...
		for _, info in pairs(luo) do
			local luoData = {}
			local luoInfo = {}
			luoInfo.type = paohuzitool.cardType.chi
			luoData[1] = info[1]
			luoData[2] = info[2]
			luoData[3] = info[3]
		...
		chiInfo.luoData = luo
		waitAction = {}
		deskInfo.actionInfo.curaction = {seat = user.seat, type = paohuzitool.cardType.chi, time = timeout[1], card = deskInfo.actionInfo.curaction.card}
...
function CMD.put(source,msg)
	...
	if ret then
	    ...
	    if deskInfo.actionInfo.iswait == 1 then
	    	...
	    	broadcastDesk(cjson.encode(noty_retobj))
	    	if #new_card == 0 then
	    		hupaiBance(user.seat,card,paohuzitool.HUPAI_TYPE.liuju)
				return PDEFINE.RET.SUCCESS
			end
			setTimeOut()
	    	draw(nextSeat)
...
function CMD.startGame() --2种开始方式  庄家设置也不一样 
	...
	notify_retobj.code   = PDEFINE.RET.SUCCESS
    notify_retobj.gameid = PDEFINE.GAME_TYPE.ZIPAI_PAOHZ
	...
	-- 	usersCard[1] = {102,102,102,104,104,105,105,203,203,205}
	-- 	usersCard[2] = {209,102,109,104,108,205,205,230}
		
	-- else
	-- 	usersCard[1] = {209,102,109,104,108,205,205,230}
	-- 	usersCard[2] = {102,102,102,104,104,105,105,203,203,205}
		
	-- end
	-- new_card = {209,108,104,104}
	-- if #usersCard[1] == 21 then
 ...
 --        [20] = 103,
 --    	}
	-- 	usersCard[1] = {208,105,104,206,207,209,201,201,201,103,203,203}
	--  else
	--  	usersCard[1] =  {
	--  		[1] = 209,
	--         [2] = 209,
	...
	--         [20] = 103,
 --    	}
	--  	usersCard[2] = {208,105,105,205,205,205,102,101,203,201,202,101,108,108,209,206,102,107,110,209}
	--  end
 --     new_card = {209,108,209,209}
	...
	for _,user in pairs(deskInfo.users) do
		if #user.handInCards == 21 then
			...
			ok,huXi,huCards = paohuzitool.checkPaoHuZiHu(user,hupaiCards,user.handInCards[21],user.uid)
			if huXi then
				hseat = user.seat
	...
	if hseat  then
		setTimeOut()
		hupaiBance(hseat,hcard,huXi,nil,huCards)
	else
		--自动打牌
		CMD.userSetAutoState("autoPut",timeout[1]*100,deskInfo.actionInfo.nextaction.seat)
	end
...
local function autoDissolve(isTimeOut)
	...
	broadcastDesk(cjson.encode(noty_retobj))
	if isTimeOut == 1 then 
		hupaiBance(deskInfo.actionInfo.curaction.seat,deskInfo.actionInfo.curaction.card,paohuzitool.HUPAI_TYPE.liuju,true)
	else
		big_over()
	end
end
...
function CMD.hallJoin(source,uid,cluster_info,ip,lat,lng,state)
	return cs(function ()
		...
		if deskInfo.conf.distance == 1 then
			if not paohuzitool.checkDistance(lat,lng,deskInfo.users) then
				return PDEFINE.RET.ERROR.DISTANCE_EXIST
			end
		end

		if deskInfo.conf.ipcheck == 1 then
			if not paohuzitool.checkIp(ip,deskInfo.users) then
		...
		userInfo.ohterRoundHuXi = 0 --每一小局显示的胡息
...
local function localGetDeskInfo(uid,lat,lng)
	...
	for _, user in pairs(tmpDeskInfo.users) do
		if user.uid ~= uid then
			for _, menziInfo in pairs(user.menzi) do
				if menziInfo.type == paohuzitool.cardType.kan then
					menziInfo.card = 0
				end
				if menziInfo.type == paohuzitool.cardType.long then
					menziInfo.card = 0
				end
			end
	...
	tmpDeskInfo.locatingList = paohuzitool.jisuanXY(deskInfo.users)
...
function CMD.getLocatingList(source,msg)
	local recvobj = cjson.decode(msg)
	local retobj = {}
	deskInfo.locatingList = paohuzitool.jisuanXY(deskInfo.users)
	retobj.c      = math.floor(recvobj.c)
    retobj.code  = PDEFINE.RET.SUCCESS
