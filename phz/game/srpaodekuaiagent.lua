...
local deskInfo = 
{
	...
	smallState = 0,
	bigState = 0,
	gameid = PDEFINE.GAME_TYPE.PUKE_PAODEKUAI_SR,
}
...
local function big_over()
	for _,user in pairs(deskInfo.users) do
		pcall(cluster.call, user.cluster_info.server, user.cluster_info.address, "deskBack", PDEFINE.GAME_TYPE.PUKE_PAODEKUAI_SR) --释放桌子对象
	end
...
function CMD.startGame() 
    ...
    notify_retobj.gameid = PDEFINE.GAME_TYPE.PUKE_PAODEKUAI_SR
	...
	-- usersCard[1] = {0x04,0x14,0x24,0x23,0x03,0x13,0x25,0x3A,0x15,0x06,0x07,0x08,0x16,0x17,0x18}
 --    usersCard[2] = {0x09,0x19,0x29,0x3B,0x0A,0x1A,0x2A,0x0B,0x1C,0x0C,0x27,0x38,0x26,0x37,0x28}
