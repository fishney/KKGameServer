...
function UserSingleEntity:Update(record, nosync)
	...
	if not record.uid or not self.recordset[record.uid] then
		LOG_DEBUG("owner uid %d: modification uid %d: data",owner, record.uid)
	end		-- 记录不存在，返回

	local ret = skynet.call(".dbmgr", "lua", "update", self.tbname, record, self.type, nosync, true)
	if ret and nil ~= self.recordset[record.uid] then
...
-- 加载玩家数据
function UserSingleEntity:set_data_change(uid)
	skynet.call(".dbmgr", "lua", "set_data_change", self.tbname, uid)
end
